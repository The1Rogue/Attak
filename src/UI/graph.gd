...
func _draw():
	...
	for i in markers:
		...
		var textSize = font.get_string_size(str(y))
		textSize.y *= -.25
		draw_string(font, Vector2(-.03, v) * size - textSize, str(y))
		
		var date = Time.get_date_string_from_unix_time(x).substr(5)
		textSize = font.get_string_size(date)
		textSize.y *= -1
		textSize.x *= .5
		draw_string(font, Vector2(v, 1.03) * size - textSize, date)
